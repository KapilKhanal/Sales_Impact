...
if __name__ == '__main__':
    p = os.path.join(os.path.dirname(__file__),'../data/raw/SalesDatabase.db')
    conn = create_connection(p)
    cur = conn.cursor()
    process_customer_table(cur, '../data/raw/CustomerTable.csv')
    conn.commit()
    process_transaction_table(cur, '../data/raw/TransactionsTable.csv')
    conn.commit()
    process_items_table(cur, '../data/raw/ItemsTable.csv')
    conn.commit()